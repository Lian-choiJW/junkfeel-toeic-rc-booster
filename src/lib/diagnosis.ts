...
export function diagnoseScore(input: DiagnosisInput): DiagnosisResult {
  ...
  if (input.rcScore < 250) {
    ...
    level = "RC 집중 상승형";
    priorities = ["Part 7 정답 근거 찾기", "Part 5 문법 정확도", "Part 6 문맥 연결", "패러프레이징 표현", "시간관리"];
  } else if (input.rcScore <= 380) {
    level = "RC 속도 개선형";
    priorities = ["Part 6 문맥", "Part 7 속도 훈련", "오답 유형 분류", "어휘 확장"];
  }

  return {
    level,
    summary: isRcFocused
      ? `현재 RC 점수는 ${input.rcScore}점입니다. LC 대비 RC가 낮기 때문에 총점 상승의 핵심은 RC입니다.`
      : `현재 RC 점수는 ${input.rcScore}점입니다. RC를 더 안정적으로 만들면 목표 점수 ${input.targetScore}점에 가까워질 수 있습니다.`,
    priorities,
    routine: [
      `하루 ${input.dailyMinutes || 40}분 기준으로 단어, Part 5, Part 7을 나누어 학습하세요.`,
      `목표 RC ${input.targetRcScore || input.rcScore + 50}점까지는 틀린 태그를 중심으로 반복하세요.`,
      "오답은 바로 지우지 말고 근거 문장과 헷갈린 이유를 함께 남기세요."
    ]
  };
}
