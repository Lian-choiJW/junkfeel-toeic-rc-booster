...
export function Part67PassageCard({ item, onNext, onAnswered }: { item: Part67Question; onNext: () => void; onAnswered: () => void }) {
  ...
  return (
    <section className="rounded-md border border-line bg-white p-5 shadow-soft">
      <p className="text-sm font-black text-ocean">Part {item.part} · {item.passageType} · {item.difficulty}</p>
      {item.scanStrategy ? (
        <div className="mt-3 rounded-md border border-teal-100 bg-teal-50 p-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-md bg-white px-2 py-1 text-xs font-black text-teal-700">Scan</span>
            <span className="text-sm font-black text-teal-950">{item.questionFocus || "근거 찾기"}</span>
          </div>
          <p className="mt-2 text-sm font-semibold leading-6 text-teal-900">{item.scanStrategy.trigger}</p>
          <div className="mt-3 grid gap-2 text-sm font-semibold text-teal-950 md:grid-cols-3">
            {item.scanStrategy.who ? <p className="rounded-md bg-white px-3 py-2">Who: {item.scanStrategy.who}</p> : null}
            {item.scanStrategy.why ? <p className="rounded-md bg-white px-3 py-2">Why: {item.scanStrategy.why}</p> : null}
            {item.scanStrategy.what ? <p className="rounded-md bg-white px-3 py-2">What: {item.scanStrategy.what}</p> : null}
          </div>
        </div>
      ) : null}
      <div className="mt-4 grid gap-4 rounded-md border border-line bg-slate-50 p-4">
      ...
      <h2 className="mt-5 text-xl font-black text-ink">{item.question}</h2>
          ...
          <div className="mt-4 rounded-md border border-line bg-white p-4">
            ...
            <div className="mt-3 grid gap-2">
              {item.paraphrase.map((row) => (
                <p key={row.passageExpression} className="rounded-md bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-900">
            ...
            <div className="mt-4 rounded-md border border-line bg-white p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {item.linkageClues.map((clue) => (
                  <span key={clue} className="rounded-md bg-teal-50 px-3 py-2 text-sm font-bold text-teal-900">
                    {clue}
                  </span>
                ))}
              </div>
